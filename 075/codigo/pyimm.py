...
class RGB(object):
	...
	def scaledRGB(self, p, q):
		"""
			RGB Integer Interval: [0, 255]
			Scaled Interval: [p, q] -> f_p : A -> [0, 255] | A = [1, p] and 
			f_q : B -> [0, 255 ] | B = [0, q]
			x \in [p, q], x / ( q - p) 

		"""
		self.r = int( ( self.r / ( q - p) - q )*255 ) # x \in [p, q] to x' \in [0, 255]
		self.g = int( ( self.g / ( q - p) - q )*255 )
		self.b = int( ( self.b / ( q - p) - q )*255 )
